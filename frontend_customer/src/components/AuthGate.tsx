 ...
 * Includes ToS acceptance on register, and global 401 session-expiry handling.
 */
import { useState, useEffect, createContext, useContext } from 'react'
import { Activity, Eye, EyeOff, RefreshCw, UserPlus, LogIn, Mail, KeyRound, ArrowLeft, FileText, ExternalLink } from 'lucide-react'
...
// Global session-expiry signal — set to true to force logout UI
let _sessionExpiredSignal: (() => void) | null = null

// Inject X-Session-Token into all /api/ requests (except /api/auth/),
// and handle 401 responses by triggering session-expiry UI.
...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => { _sessionExpiredSignal = null }
  }, [])
  ...
  const reset = (m: Mode) => {
    setMode(m); setError(''); setInfo(''); setOtp('')
    if (m !== 'register') setTosAccepted(false)
  ...
  const submitAuth = async () => {
    ...
    try {
      const body = mode === 'login'
        ? { email, password }
        : { email, password, full_name: name, tos_accepted: tosAccepted }
  ...
  if (token) return <AuthContext.Provider value={{ token, logout }}>{children}</AuthContext.Provider>
    ...
    <div className="h-screen flex items-center justify-center bg-bg-primary">
      ...
      {tosOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-[480px] max-h-[70vh] bg-bg-secondary border border-border-subtle rounded-2xl flex flex-col">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-border-subtle">
              <FileText size={15} className="text-brand-blue" />
              <span className="text-sm font-semibold text-text-primary">Terms of Service</span>
              <button onClick={() => setTosOpen(false)} className="ml-auto text-text-muted hover:text-text-primary text-lg leading-none">×</button>
            </div>
            <div className="overflow-y-auto px-5 py-4 text-xs text-text-secondary space-y-3 flex-1">
              <p><strong>1. Acceptance.</strong> By using BatteryOS you agree to these terms.</p>
              <p><strong>2. License.</strong> Your license is personal, non-transferable, and limited to the number of seats purchased.</p>
              <p><strong>3. Data.</strong> You retain ownership of your battery data. We process it solely to provide the service.</p>
              <p><strong>4. Prohibited uses.</strong> You may not reverse-engineer, resell, or use the service for illegal purposes.</p>
              <p><strong>5. Warranty.</strong> The service is provided "as is" without warranty of any kind.</p>
              <p><strong>6. Liability.</strong> To the maximum extent permitted by law, BatteryOS is not liable for indirect or consequential damages.</p>
              <p><strong>7. Governing law.</strong> These terms are governed by Indian law.</p>
              <p><strong>8. Contact.</strong> For questions: support@batteryos.io</p>
            </div>
            <div className="px-5 py-3 border-t border-border-subtle flex justify-end">
              <button onClick={() => { setTosAccepted(true); setTosOpen(false) }}
                className="px-4 py-1.5 bg-brand-blue text-white text-xs font-semibold rounded-lg hover:bg-blue-500 transition-colors">
                Accept & Close
              </button>
            </div>
          </div>
      ...
      <div className="w-80 space-y-5">
        ...
        {sessionExpired && (
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-2.5 text-xs text-amber-400 text-center">
          ...
          <div className="bg-bg-secondary border border-border-subtle rounded-2xl p-6 space-y-3">
            ...
            {mode === 'register' && (
              <label className="flex items-start gap-2 text-xs text-text-muted cursor-pointer">
                ...
                <span>
                  I agree to the{' '}
                  <button type="button" onClick={() => setTosOpen(true)}
            ...
            <button onClick={submitAuth} disabled={loading || !password || (mode === 'register' && !tosAccepted)}
